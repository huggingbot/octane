...
export function getWhirlpoolsContext(connection: Connection): WhirlpoolContext {
  ...
  return WhirlpoolContext.from(connection, wallet as any, WHIRLPOOL_PROGRAM_ID)
}
...
export async function getSwapInstructions(
  ...
  const setupInstructions = [createAssociatedTokenAccountInstruction(feePayer, associatedSOLAddress, user, NATIVE_MINT)]
